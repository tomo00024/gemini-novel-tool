...
<div class="flex flex-col h-[100dvh] p-4 overflow-hidden">
	<!-- ヘッダー部分 -->
	...
	<!-- チャットログ（この部分がスクロールする） -->
	<!-- flex-1 で残りの空間を全て埋め、overflow-y-auto で内容が多い時だけスクロールさせる -->
				...
				<div class="chat-bubble group {message.speaker === 'user' ? 'chat-bubble-primary' : ''}">
					{#if editingId === message.timestamp}
						<div class="flex flex-col gap-2" style="min-width: {editingBubbleWidth}px;">
							<button class="close-edit-button" title="編集をキャンセル" on:click={cancelEditing} >
							...
							<textarea bind:this={textareaElement} bind:value={editingText} class="textarea textarea-bordered w-full" on:input={adjustTextareaHeight} on:keydown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); saveEditing(); } }} ></textarea>
					...
					{#if editingId !== message.timestamp}
						<div class="floating-menu">
							<button class="menu-button" title="編集" on:click={(event) => startEditing(message, event)} >
