...
<script lang="ts">
	...
	// ★★★ ここからがUIモード切替のための新しい関数 ★★★
	/**
	 * UI表示モードのトグルが変更されたときに呼び出される
	 * @param event DOMイベント
	 */
	function handleViewModeChange(event: Event) {
		const isChecked = (event.currentTarget as HTMLInputElement).checked;
		const newMode = isChecked ? 'game' : 'standard';

		sessions.update(allSessions => {
			const sessionToUpdate = allSessions.find(s => s.id === $page.params.id);
			if (sessionToUpdate) {
				// featureSettingsオブジェクトが存在しない場合に備えて初期化
				if (!sessionToUpdate.featureSettings) {
					sessionToUpdate.featureSettings = {};
				}
				sessionToUpdate.featureSettings.chatViewMode = newMode;
				sessionToUpdate.lastUpdatedAt = new Date().toISOString();
			}
			return allSessions;
		});
...
<div class="flex flex-col h-screen p-4">
	...
	{#if $currentSession}
		<div class="space-y-6">
			...
			<!-- ★★★ ここからがUIモード設定の新しいUIブロック ★★★ -->
			<div class="p-4 border rounded-lg">
				<h2 class="text-lg font-semibold mb-3">表示設定</h2>
				<label class="flex items-center justify-between cursor-pointer">
					<span>ゲーム風UIで表示する</span>
					<input
						type="checkbox"
						class="toggle"
						checked={$currentSession.featureSettings?.chatViewMode === 'game'}
