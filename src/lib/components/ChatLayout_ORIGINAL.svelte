...
<div class="flex h-[100dvh] flex-col overflow-hidden p-4">
	<div class="flex-shrink-0">
		...
		<div class="mb-4 flex items-center gap-4">
			<a
				href="{base}/"
				class="flex-shrink-0 rounded bg-stone-200 px-3 py-2 text-sm font-semibold text-text-sub hover:bg-bg-hover"
			>
				螻･豁ｴ縺ｫ謌ｻ繧・
			</a>
			<div class="min-w-0 flex-1">
				{#if isEditing}
					<input
						...
						on:blur={saveTitle}
						class="text-m w-full rounded-md border border-stone-400 bg-white px-2 py-1 font-bold text-text-sub focus:ring-2 focus:ring-blue-500 focus:outline-none"
					/>
				{:else}
					<button
						type="button"
						on:click={startEditing}
						class="text-m w-full truncate rounded-md px-2 py-1 text-left font-bold text-text-sub hover:bg-bg-hover focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
						title={sessionTitle}
					>
						{sessionTitle}
					</button>
			...
			<div class="flex flex-shrink-0 items-center gap-4">
				<a
					...
					class="rounded bg-stone-200 px-3 py-2 text-sm font-semibold text-text-sub hover:bg-bg-hover"
				>
					繧｢繝励Μ險ｭ螳・
				</a>
				<a
